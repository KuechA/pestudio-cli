...
class VirusTotalClient:
    ...
    def sendRequest(self):
        ...
        if response.status_code == 200:  # Response: OK
            ...
            logger.info("Sent: %s, Response: %s", os.path.basename(
                self.file), str(json_response))
            self.resourceId = json_response['resource']
            return self.resourceId
        else:
            logger.warning("Sent: %s, HTTP: %d", os.path.basename(
                self.file), response.status_code)
            return None
    ...
    def getReport(self, resourceId=None):
        ...
        headers = {"Accept-Encoding": "gzip, deflate",
                   "User-Agent": "VirusTotalClient"}
        response = requests.get(
            'https://www.virustotal.com/vtapi/v2/file/report', params=params, headers=headers)
        if response.status_code == 200:  # Response: OK
    ...
    def printReport(self, showPositiveResultsOnly=True):
        ...
        resultString = "VirusTotal result: " + \
            str(self.report['positives']) + " of " + \
            str(self.report['total']) + " tests are positive\n"
        if showPositiveResultsOnly and not self.report['positives'] == 0:
            ...
            for test, result in self.report["scans"].items():
                if showPositiveResultsOnly:
                    if result['detected']:
                        table.add_row(
                            [test, result['version'], result['result']])
                else:
    ...
    def getJsonReport(self, jsonDict):
        ...
        VirusTotal_result = {"summary": {"positives": str(
            self.report['positives']), "total": str(self.report['total'])}}
        VirusTotal_result["details"] = []
        for test, result in self.report["scans"].items():
            VirusTotal_result["details"].append({"engine": str(test), "version": str(
                result['version']), "result": str(result['result'])})

        jsonDict["VirusTotal"] = VirusTotal_result
        return jsonDict
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--file", help="The file to scan", required=True, dest="file")
