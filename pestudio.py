...
import json
import readline
import glob  # to path auto complete
import pydoc
import datetime
import os
...
def parseCommandLineArguments():
    parser = argparse.ArgumentParser(
        description='PE file analyzer. The default output is human-readable and structured in tables. If no file is specifies, the interactive mode is entered.')
    parser.add_argument(
        "-f", "--file", help="The file to analyze", required=False, dest="file")
    parser.add_argument(
        "-y", "--yara", help="Use yara to check for more signatures. Use: --yara <yara-file>", required=False, dest="yara")
    parser.add_argument("-v", "--virusTotal", help="Submit the file to virus total and get their score.",
                        action="store_true", dest="virusTotal")
    parser.add_argument("--header", help="Show information from header.",
                        action="store_true", dest="header")
    parser.add_argument("-t", "--tlsCallbacks",
                        help="Show addresses of TLS callbacks.", action="store_true", dest="tls")
    parser.add_argument("-i", "--imports", help="Check the imports against known malicious functions.",
                        action="store_true", dest="imports")
    parser.add_argument("--indicators", help="Check the indicators of the file.",
                        action="store_true", dest="indicators")
    parser.add_argument("-e", "--exports", help="Show the exports of the binary",
                        action="store_true", dest="exports")
    parser.add_argument("-r", "--resources", help="Check the resources for blacklisted values.",
                        action="store_true", dest="resources")
    parser.add_argument("--relocations", help="Show the relocations.",
                        action="store_true", dest="relocations")
    parser.add_argument("-s", "--signatures", help="Check for known signatures (e.g. packers).",
                        action="store_true", dest="signatures")
    parser.add_argument("--strings", help="Check the strings in the PE file.",
                        action="store_true", dest="strings")
    parser.add_argument("--script", help="Indicates whether the tool is called by a script.",
                        action="store_true", dest="inScript")
    parser.add_argument(
        "-u", "--urls", help="List all URLs found in the PE file", action="store_true", dest="urls")
    parser.add_argument(
        "-x", "--xml", help="Format output as xml.", action="store_true", dest="xml")
    parser.add_argument(
        "-j", "--json", help="Format output as JSON.", action="store_true", dest="json")
    parser.add_argument("--interactive", help="Use the tool in interactive mode.",
                        action="store_true", dest="interactive")
    parser.add_argument("-d", "--dump_resources",
                        help="Dump every resource to a file (with name or ID as filename) in folder ./resources.", action="store_true", dest="dumpRes")
    return parser.parse_args()


def parseIndicators():
    indicators = {}
    root = ET.parse(path+"xml/indicators.xml").getroot().find('indicators')
    for indicator in root.findall('indicator'):
        indicators[indicator.attrib['id']] = Indicator(
            indicator.attrib['enable'], indicator.attrib['severity'], indicator.attrib['id'], indicator.text)
...
def collectIndicators(vt, peAnalyzer, matcher, all=False, root=None, jsonDict=None):
    ...
    # VirusTotal result
    try:
        rootThresholds = ET.parse(path+"xml/thresholds.xml").getroot()
        ...
        vtRes += "VirusTotal result: " + str(vt.report['positives']) + " of " + str(
            vt.report['total']) + " tests are positive" + constants.RESET
        ...
        if jsonDict is None and root is None:
            print(constants.BLUE +
                  "\tNo connection to VirusTotal possible" + constants.RESET)

    if not jsonDict is None:
        score, maxScore, jsonDict, root = peAnalyzer.printIndicators(
            indicators, score, maxScore, table, all=all, jsonDict=jsonDict)
    elif not root is None:
        score, maxScore, jsonDict, root = peAnalyzer.printIndicators(
            indicators, score, maxScore, table, all=all, root=root)
    else:
        score, maxScore, jsonDict, root = peAnalyzer.printIndicators(
            indicators, score, maxScore, table, all=all)

    if not jsonDict is None:
        score, maxScore, jsonDict, root = peAnalyzer.checkFeatures(
            indicators, score, maxScore, table, jsonDict=jsonDict)
    elif not root is None:
        score, maxScore, jsonDict, root = peAnalyzer.checkFeatures(
            indicators, score, maxScore, table, root=root)
    else:
        score, maxScore, jsonDict, root = peAnalyzer.checkFeatures(
            indicators, score, maxScore, table)
    ...
    if jsonDict is None and root is None:  # print summary only for user interface mode
        ...
        timeDateStamp = datetime.datetime.fromtimestamp(
            peAnalyzer.peFile.header.time_date_stamps)
        if timeDateStamp > datetime.datetime.now():
            print("%s\tFile Header: Suspicious value for TimeDateStamp (%s)%s" % (
                constants.RED, str(timeDateStamp), constants.RESET))
        elif all:
            print(constants.GREEN +
                  "\tFile Header seems to be valid" + constants.RESET)
        ...
        if not peAnalyzer.checkImportNumber():
            print(constants.GREEN + "\tNumber of imports is in a reasonable range (%d)" %
                  len(peAnalyzer.imports), constants.RESET)
        elif all:
        ...
        if len(suspicious):
            print(constants.RED + "\t%d out of %d imports are blacklisted" %
                  (len(suspicious), len(peAnalyzer.imports)) + constants.RESET)
        elif all:
            print(constants.GREEN +
                  "\tNo blacklisted imports found" + constants.RESET)

        # Blacklisted resources
        resources = peAnalyzer.blacklistedResources()
        if len(resources):
            print(constants.RED + "\t%d blacklisted resources found" %
                  (len(resources)) + constants.RESET)
        elif all:
            print(constants.GREEN +
                  "\tNo blacklisted resources found" + constants.RESET)

        # tls callbacks
        if peAnalyzer.peFile.has_tls:
            print(constants.RED +
                  "\tThe PE file uses TLS callbacks." + constants.RESET)
        elif all:
            print(constants.GREEN +
                  "\tNo TLS callbacks used PE file" + constants.RESET)
        ...
        if blacklisted > 0:
            print(constants.RED + "\t%d strings are blacklisted" %
                  (blacklisted) + constants.RESET)
        elif all:
            print(constants.GREEN +
        ...
        if len(packers):
            print(constants.RED + "\tThe signature of the following packer was found: " +
                  str(packers) + constants.RESET)
        elif all:
            print(constants.GREEN +
                  "\tNo packer signature was found in the PE file" + constants.RESET)

    # end if: summary only for user interface mode

    if not jsonDict is None:
        jsonDict["indicators"]["summary"] = {
            "Severity": str(score), "MaxSeverity": str(maxScore)}
    elif not root is None:
        ...
        table.field_names = [
            "Description", "Severity(" + str(score) + "/" + str(maxScore) + ")"]
        print(table)
...
def checkYara(file, yarafile, root=None, jsonDict=None):
    ...
    if not os.path.isfile(yarafile):
        if root is None and jsonDict is None:
            print(constants.BLUE + "Could not find the specified file %s" %
                  yarafile + constants.RESET)
        elif root is None:
        ...
        if root is None and jsonDict is None:
            if matches != []:
                print(constants.RED + "Yara found the following %d matches:" %
                      len(matches) + constants.RESET)
...
def interactiveMode(file=None):
    ...
    while user_in != "q" and user_in != "quit":
        if user_in.startswith("file ") or user_in.startswith("f "):
            ...
            if len(args) > 2:
                print(
                    "Please use the command only with one argument: yara|y <rule-file>")
            else:
                yarafile = args[1]
            ...
            print("Blacklisted resources found: " + str(blacklistedResources)
                  if len(blacklistedResources) > 0 else "No blacklisted resources found")
            ...
            if len(packers):
                ...
                print(
                    constants.GREEN + "No packer signature was found in the PE file" + constants.RESET)
            ...
            print(
                "\v/virusTotal - submit the file to VirusTotal and display a summary of the result")
            ...
            print(
                "\tindicators -a - show indicators of malware in the PE file (show all checks)")
            ...
            print(
                "\ts/signatures - find signatures of malicious patterns or packers in the PE file")
...
def checkFile(args):
    ...
    if args.indicators:
        if args.xml:
            jsonDict, root = collectIndicators(
                vt, peAnalyzer, matcher, root=root)
        elif args.json:
            jsonDict, root = collectIndicators(
                vt, peAnalyzer, matcher, jsonDict=jsonDict)
        else:
            collectIndicators(vt, peAnalyzer, matcher)
